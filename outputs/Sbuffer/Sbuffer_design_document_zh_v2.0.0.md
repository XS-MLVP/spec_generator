...
| 文档版本 | v2.0.0 |
| 使用模板版本 | v2.0.0 |
| 前一版本 | [v1.0.0](./Sbuffer_design_document_zh_v1.0.0.md) |
| 版本变更类型 | Major：模板 I/O schema 不兼容升级，新增 Chisel 存在性、当前配置生成状态和裁剪依据列；设计行为不变 |
...
| Elaborated Verilog 顶层 | `Sbuffer` / [`manifest.json`](../../evidence/Sbuffer/v2.0.0/manifest.json)、[`ports.csv`](../../evidence/Sbuffer/v2.0.0/ports.csv) |
...
| 生成命令 | `tools/generate_rtl.sh --module Sbuffer --config DefaultConfig --version v2.0.0` |
| RTL 证据 | [`manifest.json`](../../evidence/Sbuffer/v2.0.0/manifest.json)、[`ports.csv`](../../evidence/Sbuffer/v2.0.0/ports.csv)；SHA-256 `1e1fe1c1fcea11b0e016dfce391dd771c62eff310cdb4702e712d5a9407e027e`；166 ports |
| 图形渲染证据 | [`diagrams/manifest.json`](../../evidence/Sbuffer/v2.0.0/diagrams/manifest.json)；Mermaid CLI 11.16.0；3 张 SVG 均实际渲染成功 |
...
| FACT-006 | DefaultConfig elaborated `Sbuffer` 有 166 个叶端口：37 input、129 output。 | [`ports.csv`](../../evidence/Sbuffer/v2.0.0/ports.csv)、[`manifest.json`](../../evidence/Sbuffer/v2.0.0/manifest.json) | 已确认 | Closed |
...
下表中的数组模式已逐项核对 v2 [`ports.csv`](../../evidence/Sbuffer/v2.0.0/ports.csv)。`i=0..1`、`k=0..2`、`b=0..15`、`p=0..15`；`Elided` 表示 Chisel 字段存在，但在 DefaultConfig 产物中经 feature disable、常量传播或 dead-port elimination 消失。

| Bundle class | Chisel 对象 / 字段 | Chisel 存在 | 方向 | Chisel 类型 / 位宽 | 当前配置生成状态 | 精确 Verilog I/O | Verilog 位宽 | 裁剪 / 生成依据 | 协议 / 对端 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| module implicit | `clock` | 是 | I | `Clock` | Generated | `clock` | 1 | `ports.csv` index 0 | 主时钟 |
| module implicit | `reset` | 是 | I | `Reset` | Generated | `reset` | 1 | `ports.csv` index 1 | 同步寄存器复位 |
| `SbufferWriteIO` | `io.in.req[i].ready` | 是 | O | `Bool` | Generated | `io_in_req_[i]_ready` | 1 | manifest + contiguous index match | Store Queue Decoupled |
| `SbufferWriteIO` | `io.in.req[i].valid` | 是 | I | `Bool` | Generated | `io_in_req_[i]_valid` | 1 | manifest + contiguous index match | Store Queue Decoupled |
| `DCacheWordReqWithVaddrAndPfFlag` | `io.in.req[i].bits.vaddr` | 是 | I | `UInt(50.W)` | Generated | `io_in_req_[i]_bits_vaddr` | 50 | `ports.csv` | CAM vtag / Store Queue |
| 同上 | `.bits.data` / `.bits.mask` | 是 | I | `UInt(128.W)` / `UInt(16.W)` | Generated | `io_in_req_[i]_bits_data` / `io_in_req_[i]_bits_mask` | 128 / 16 | `ports.csv` | SbufferData write |
| 同上 | `.bits.addr` / `.bits.wline` | 是 | I | `UInt(48.W)` / `Bool` | Generated | `io_in_req_[i]_bits_addr` / `io_in_req_[i]_bits_wline` | 48 / 1 | `ports.csv` | ptag、offset、整行零写 |
| 同上 | `.bits.{cmd,vaddr_dup,id,instrtype,isFirstIssue,replayCarry,lqIdx,debug_robIdx,prefetch,vecValid,sqNeedDeq}` | 是 | I | 见 [`DCacheWrapper.scala:421-469`](../../third_party/XiangShan/src/main/scala/xiangshan/cache/dcache/DCacheWrapper.scala#L421) | Elided | 未生成 | 0 | Scala 存在；`ports.csv` 无对应叶端口 | 不可作为本配置 bind 端口 |
| `DCacheToSbufferIO` | `io.dcache.req.ready` / `.valid` | 是 | I / O | `Bool` | Generated | `io_dcache_req_ready` / `io_dcache_req_valid` | 1 / 1 | `ports.csv` | DCache Decoupled |
| `DCacheLineReq` | `io.dcache.req.bits.vaddr` / `.addr` | 是 | O | 50 / 48 bits | Generated | `io_dcache_req_bits_vaddr` / `io_dcache_req_bits_addr` | 50 / 48 | `ports.csv` | line address / DCache |
| 同上 | `.bits.data` / `.mask` / `.id` | 是 | O | 512 / 64 / 6 bits | Generated | `io_dcache_req_bits_data` / `io_dcache_req_bits_mask` / `io_dcache_req_bits_id` | 512 / 64 / 6 | `ports.csv` | line payload + entry ID |
| 同上 | `.bits.cmd` | 是 | O | `UInt(5.W)`，恒为 `M_XWR` | Elided | 未生成 | 0 | constant-folded；Scala assignment `Sbuffer.scala:700` | DCache command 语义 |
| `DCacheLineResp` | `io.dcache.main_pipe_hit_resp.valid` / `.bits.id` | 是 | I | 1 / 6 bits | Generated | `io_dcache_main_pipe_hit_resp_valid` / `io_dcache_main_pipe_hit_resp_bits_id` | 1 / 6 | `ports.csv` | completion / DCache MainPipe |
| 同上 | `io.dcache.replay_resp.valid` / `.bits.id` | 是 | I | 1 / 6 bits | Generated | `io_dcache_replay_resp_valid` / `io_dcache_replay_resp_bits_id` | 1 / 6 | `ports.csv` | replay / DCache MainPipe |
| 同上 | 两响应的 `.bits.{data,miss,replay}` | 是 | I | 512 / 1 / 1 bits | Elided | 未生成 | 0 | 本配置功能逻辑仅保留 ID；assert/difftest 被裁剪 | DCache response payload |
| `SbufferForward` | `io.forward[k].s0Req.valid` / `.bits.vaddr` | 是 | I | 1 / 50 bits | Generated | `io_forward_[k]_s0Req_valid` / `io_forward_[k]_s0Req_bits_vaddr` | 1 / 50 | `ports.csv` contiguous `k` | Load S0 |
| 同上 | `.s1Req.paddr` / `.s1Kill` | 是 | I | 48 / 1 bits | Generated | `io_forward_[k]_s1Req_paddr` / `io_forward_[k]_s1Kill` | 48 / 1 | `ports.csv` contiguous `k` | Load S1 |
| 同上 | `.s2Resp.valid` | 是 | O | `Bool` | Generated | `io_forward_[k]_s2Resp_valid` | 1 | `ports.csv` contiguous `k` | Load S2 |
| `SbufferForwardResp` | `.s2Resp.bits.forwardMask[b]` | 是 | O | `Vec(16,Bool)` | Generated | `io_forward_[k]_s2Resp_bits_forwardMask_[b]` | 每项 1 | `ports.csv` contiguous `k/b` | byte mask / Load pipeline |
| 同上 | `.s2Resp.bits.forwardData[b]` | 是 | O | `Vec(16,UInt(8.W))` | Generated | `io_forward_[k]_s2Resp_bits_forwardData_[b]` | 每项 8 | `ports.csv` contiguous `k/b` | byte data / Load pipeline |
| 同上 | `.s2Resp.bits.matchInvalid` | 是 | O | `Bool` | Generated | `io_forward_[k]_s2Resp_bits_matchInvalid` | 1 | `ports.csv` contiguous `k` | address consistency |
| top scalar | `io.sqempty` / `io.mshr_store_empty` | 是 | I | `Bool` | Generated | `io_sqempty` / `io_mshr_store_empty` | 1 / 1 | `ports.csv` | empty qualification |
| top scalar | `io.sbempty` / `io.sbFull` | 是 | O | `Bool` | Generated | `io_sbempty` / `io_sbFull` | 1 / 1 | `ports.csv` | registered status |
| `SbufferFlushBundle` | `io.flush.valid` / `.isCmo` / `.empty` | 是 | I / I / O | `Bool` | Generated | `io_flush_valid` / `io_flush_isCmo` / `io_flush_empty` | 1 each | `ports.csv` | flush controller |
| `CustomCSRCtrlIO` | `io.csrCtrl.sbuffer_timeout` | 是 | I | `UInt(22.W)` | Generated | `io_csrCtrl_sbuffer_timeout` | 22 | `ports.csv` | coherence timeout |
| top scalar | `io.force_write` | 是 | I | `Bool` | Generated | `io_force_write` | 1 | `ports.csv` | threshold reduction |
| top scalar | `io.hartId` / `io.physicalStoreQueueFull` | 是 | I | `UInt(6.W)` / `Bool` | Elided | 未生成 | 0 | Difftest disabled；perf-only input optimized | Core / PMU |
| prefetch | `io.memSetPattenDetected` / `io.store_prefetch[0..1]` | 是 | mixed | `Bool` / `Decoupled[StorePrefetchReq]` | Elided | 未生成 | 0 | SPB 与 commit-prefetch disabled | Prefetch consumer |
| `DiffStoreIO` | `io.diffStore` | 是 | I | aggregate | Elided | 未生成 | 0 | `EnableDifftest=false` | Difftest |
| `CustomCSRCtrlIO` | 除 `sbuffer_timeout` 外字段 | 是 | I | aggregate leaves | Elided | 未生成 | 0 | DUT 未读取 / dead-port elimination | CSR control |
| `PerfEvent` | `io_perf[p].value` | 是 | O | `UInt(6.W)` | Generated | `io_perf_[p]_value` | 6 | `ports.csv` contiguous `p` | 16 路 PMU event |
...
渲染证据：[顶层状态图 SVG](../../evidence/Sbuffer/v2.0.0/diagrams/diagram-01-stateDiagram-v2.svg)。
...
flowchart TB
    ...
    subgraph DUT["DUT: Sbuffer"]
        ...
        PERF[performance event registers]
    ...
    SQ -->|enqueue requests 0 and 1| CAM
    OUT -->|DCache line request| DC
    DC -->|completion or replay response| META
    DC -.->|completion clears entry mask| DATA
    LD -->|forward stage 0 and stage 1 request| FWD
    FWD -->|forward stage 2 response| LD
    FC -->|flush and empty inputs| FSM
    FSM -->|empty and full status| FC
    CSR -->|timeout and force write control| CNT
    PERF -->|performance events 0 through 15| PMU
```

图中所有跨边界信号均出现在 I/O 表。DefaultConfig 中 prefetch 与 Difftest 端口被消除，因此未画为有效边界。

渲染证据：[微架构图 SVG](../../evidence/Sbuffer/v2.0.0/diagrams/diagram-02-flowchart.svg)。图内使用人类可读接口描述；精确 Chisel/Verilog 数组模式保留在 I/O 表，避免 Mermaid 将 `[i]` 等文本误解析为语法。
...
sequenceDiagram
    ...
    SQ->>SB: Enqueue valid and payload
    SB-->>SQ: Enqueue ready
    SB->>SB: Accept and update entry metadata
    SB->>SD: Internal data write request
    SD->>SD: Update selected bytes on next edge
    SB->>SB: Select entry and mark it inflight
    SB->>DC: Send line write request when unblocked
    alt Completion including accepted miss
        DC-->>SB: Main pipe completion and entry ID
        SB->>SB: invalidate entry
        SB->>SD: Clear entry mask on next edge
    else Replay response
        DC-->>SB: Replay and entry ID
        SB->>SB: Retain inflight entry and count upward
        SB->>DC: Retry after replay timeout
    end
```

渲染证据：[事务时序图 SVG](../../evidence/Sbuffer/v2.0.0/diagrams/diagram-03-sequenceDiagram.svg)。
