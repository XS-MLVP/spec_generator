...
## 2026-09-03 阅读层次与单一事实源修订

模板结构版本由 v2.1.1 升为 v3.0.0。该 Major 重构将文档拆分为正文、验证计划和附录：正文先建立生产者、消费者、关键概念和端到端数据流模型；验证计划承载 FC、CK、Coverage 和 Test Plan；完整端口、配置裁剪、证据与签核移入附录。正文采用逻辑接口名，并以 `P-*` 作为行为的唯一权威定义，后续内容通过 ID 引用，避免同一事实在多个表格中重复陈述。

## 2026-09-03 表达职责与实例分层修订

模板结构版本由 v3.0.0 升为 v3.1.0。该 Minor 保留三层信息架构和 UCAgent 标签协议，进一步区分表达方式及抽象层次：

| 问题 | v3.1.0 修订 | 校验方式 |
| --- | --- | --- |
| 正文连续出现形态相近的表格，主结论和审计信息难以区分。 | 摘要、设计原理和验证策略改用短段落；拓扑和生命周期使用小图；行为规则使用公式或伪代码；表格只承担映射、能力矩阵、Test Plan 和附录清单。 | v3.1 checker 要求摘要字段、行为小节字段和伪代码块存在，并禁止 FC/CK 注册表进入主要阅读路径。 |
| FC 表和 CK 表职责重叠。 | 验证执行统一到一张 Test Plan；FC/CK 完整注册表移至附录 F，FC 只定义验证目标，CK 只定义单一可执行性质。 | checker 验证每个 FC/CK 都出现在 Test Plan，并核对 CK 所属 FC、FC 所属 FG 及 API/Coverage Style。 |
| 模块级统一规则与通道实例特例混写。 | 每项 `P-*` 先定义统一函数，再由实例能力矩阵描述通道、bank、pipe 或 entry 类别差异；精确实例和裁剪详情放附录 C。 | checker 要求实例能力矩阵，以及每个 `P-*` 的输入、输出、延迟、适用实例、边界和伪代码。 |
| 源码行号和 RTL 定位打断正文。 | 正文只引用 `[E-*]`，附录 D 展开路径、行号、commit 和配置。 | checker 检查 E-ID 定义唯一、引用可解析，并禁止正文出现原始 `path:line`。 |

该版本不为特定 DUT 固化 forward、bypass 或 RegCache 章节；这些能力作为实例矩阵的可选维度，适用于执行通道，也可替换为 cache bank、MMU port、queue class 或其他模块特有能力。
